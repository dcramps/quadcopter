...
float OutScaleMin = -50; // the top and 
float OutScaleMax = 50;  // bottom trends
...
void setup()
{
  ...
  controlP5.addButton("ToggleMotors",0.0,10,340,120,20);
...
void ToggleMotors()
{
  if (motorsEnabled==true) {
    motorsEnabled = false;
    PField.setText(Float.toString(lastP));
    IField.setText(Float.toString(lastI));
    DField.setText(Float.toString(lastD));
    Send();
  } else {
    motorsEnabled = true;
    PField.setText(Float.toString(lastP));
